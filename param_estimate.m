...
%function = [(m2*c2^2 + l1*m2*c2 + I2), - c2*g*m2] * [ddtheta1;sin(theta1)];

 %%
numvariables = 2;

xdata = linspace(1,100);
ydata = 3.2*sin(xdata) + 4.4^2*xdata;
x0=[3,4];
fun = @(x)x(1)*sin(xdata) + x(2)^2*xdata -ydata;
options = optimoptions(@lsqnonlin,'Algorithm','trust-region-reflective');
x = lsqnonlin(fun,x0,[],[],options); 

%%
%xx=[m2, c2, l1, I2, g];
theta1 = linspace(1,100);
ddtheta1 = linspace(1,100);
